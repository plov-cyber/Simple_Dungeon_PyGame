...
class Button:
    def __init__(self, x, y, width, height, txt, sc):
        self.bg = pygame.transform.scale(BTN_IMGS[0], (width, height))
        self.pos = (x, y)
        self.x = x
        self.y = y
        self.width = width
    ...
    def change_state(self):
        if self.is_focused(pygame.mouse.get_pos()):
            self.bg = pygame.transform.scale(BTN_IMGS[1], (self.width, self.height))
        else:
            self.bg = pygame.transform.scale(BTN_IMGS[0], (self.width, self.height))
        self.update()
...
BTN_IMGS = [load_image('btn.png', WHITE), load_image('btn_act.png', WHITE)]
